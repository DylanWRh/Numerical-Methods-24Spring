...
对具体事情的建模：一家银行的客户服务部门有3个柜台用于处理客户的业务。客户以平均每10分钟到达一家银行（到达率$\lambda=0.1$），每个柜台处理每个客户业务的平均时间为5分钟（服务率$\mu=0.2$）。我们想通过模拟来估计客户的平均等待时间，以及和理论值进行对比。模拟时间 = 30000分钟 (大约50小时)

根据Little's Theorem，平均等待时间为
$$
W_q = \dfrac{\lambda}{\mu (\mu-\lambda)} = 5
$$
...
得到的结果为
```
在30000.0000分钟内，共有 3084 位顾客到来。
已经服务了 3084 位顾客。
顾客的平均等待时间为 5.2326 分钟。
平均等待时间为: 5.2326
使用Little's Law计算的理论平均等待时间为: 5.0000
```
